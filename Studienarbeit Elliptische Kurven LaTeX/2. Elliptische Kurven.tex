...
\textit{Induktionsschluss} $i \rightarrow i + 1$ (\textit{mit} $i + 1 \leq l$)




Da der Fall sehr kompliziert ist, wollen wir dies nun anhand eines Beispiels vollständig durchrechnen. Gegeben sei ein Polynom einer elliptischen Kurve E der Form $$F(X, Y) = Y^{2} - X^{3} - 12 \cdot X - 17 \in \mathbb{F}_{421}$$ Stellt man die Gleichung der Funktion nach $y^{2}$ um, dann erhält man folgende Gleichung: $$y^{2} =  x^{3} + 12 \cdot x + 17 \text{ mod }421$$ Es soll geprüft werden, ob $x_0 = 50$, $x_1 = 200$ und $x_2 = 400$ auf der elliptischen Kurve E Quadrate sind. Weiterhin sollen die Wurzeln bestimmt werden, sofern es sich bei $x_0$, $x_1$ und $x_2$ um Quadrate handelt.\\
